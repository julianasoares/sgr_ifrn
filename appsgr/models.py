...
class Campus(models.Model):
    ...
    telefone = models.CharField("Telefone", max_length=11, min_length=11, null=False)

#Modelo relação diretor com campus
class DiretorCampus(models.Model):
    diretor = models.ForeignKey(Coordenador, on_delete=models.PROTECT, verbose_name="Coordenador", unique=True)
...
class Curso(models.Model):
    nome = models.CharField("Nome", max_length=50, null=False)
    periodos = models.IntegerField("Períodos", max_digits=2, null=False, default=0)
...
class Requerimento(models.Model):
    id = models.BigIntegerField("id", unique=True, primary_key=True)
    data_solicitacao_requerimento = models.DateTimeFieldField("Data da solicitação", null=False, blank=False, auto_now_add=True, editable=False)
    ...
    observacoes = models.TextFieldField("Observações", blank=True)
    data_realizacao_certificacao=models.DateField("Data da realização da certificação", null=False, blank=False)
    encaminhamentos = models.TextFieldField("Encaminhamentos", blank=True)
    ...
    periodo_trancamento = models.IntegerField("Período de Trancamento", max_digits=2, null=False, default=0)
    outros = models.TextFieldField("Outros", blank=True)
    justificava = models.TextFieldField("Justificativa", blank=True)
    observacoes_analise_aproveitamento = models.TextFieldField("Observações da análise do aproveitamento", blank=True)
    observacoes_documentos_apresentados = models.TextFieldField("Observações dos documentos apresentados", blank=True)
